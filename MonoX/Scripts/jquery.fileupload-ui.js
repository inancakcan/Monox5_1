    ...
    $.widget('blueimpUI.fileupload', $.blueimp.fileupload, {
        options: {
            ...
            add: function (e, data) {
                ...
                data.context = that._tmpl(null, data.files).find("li");
                    /*
                    that._renderUpload(data.files)
            ...
            send: function (e, data) {
                ...
                if (data.context && data.dataType &&
                    ...
                    data.context.find('.fileupload-ui-progressbar').progressbar(
                        'value',
                        parseInt(100, 10)
                    );
                }
            ...
            // Callback for successful uploads:
                ...
                var that = $(this).data("blueimpUIFileupload");
                var params = that.options.initParams;

                if (data.context) {
                    data.context.each(function (index) {
                        var file = ($.isArray(data.result) && data.result[index]) || { Message: 'emptyResult' };
                        if (file.Message) {
                            file.error = file.Message;
                            that._adjustMaxNumberOfFiles(1);
                            alert(file.Message+"\n"+file.FileName);
                        } else if (file.FileSize > 0) {
                            var fn = window[params.OnClientFileUploaded];
                            if (typeof fn === 'function') { fn(file.HandlerData[file.FileName]); }
                        }
                    ...
                    /*
                    that._renderDownload(data.result).css('display', 'none')
            ...
            stop: function () {
                ...
                var fn = window[that.options.initParams.OnClientFilesUploadFinished];
        ...
        _renderUpload: function (files) {
            ...
            tmpl.find('.start button').slice(
                ...
                .button({
                    text: false,
                    icons: { primary: 'fileupload-ui-icon-circle-arrow-e' }
                });
            tmpl.find('.cancel button').slice(1).remove().end().first()
                .button({
                    text: false,
                    icons: { primary: 'fileupload-ui-icon-cancel' }
                });
            return tmpl;
        },
        ...
        // jquery templates dependancy removed
        _tmpl: function(template, files) {
            var tpl = "<ul>";
            $.each(files, function (index, file) {
                if (file.name !== undefined) {
                    tpl += "<li>" + file.name + "</li>";
                } else {
                    tpl += "<li><a href='" + file.Url.replace('~', '') + "'>" + file.Name + "</a></li>";
                }
                return;
            });
            return $(tpl + "</ul>");
        ...
        _renderDownload: function (files) {
            ...
            tmpl.find('.delete button').button({
                text: false,
                icons: { primary: 'fileupload-ui-icon-trash' }
            });
        ...
        _initFileUploadButtonBar: function () {
            ...
            fileUploadButtonBar
                .addClass('fileupload-ui-widget-header fileupload-ui-corner-top');
            this.element.find('.fileinput-button').each(function () {
                var fileInput = $(this).find('input:file').detach();
                $(this).button({ icons: { primary: 'fileupload-ui-icon-plusthick' } })
                    .append(fileInput);
            });
            fileUploadButtonBar.find('.start')
                .button({ icons: { primary: 'fileupload-ui-icon-circle-arrow-e' } })
                .bind('click.' + ns, function (e) {
                    e.preventDefault();
                    filesList.find('.start button').click();
                });
            fileUploadButtonBar.find('.cancel')
                .button({ icons: { primary: 'fileupload-ui-icon-cancel' } })
                .bind('click.' + ns, function (e) {
                    e.preventDefault();
                    filesList.find('.cancel button').click();
                });
            fileUploadButtonBar.find('.delete')
                .button({ icons: { primary: 'fileupload-ui-icon-trash' } })
                .bind('click.' + ns, function (e) {
                    e.preventDefault();
        ...
        _destroyFileUploadButtonBar: function () {
            this.element.find('.fileupload-buttonbar')
                .removeClass('fileupload-ui-widget-header fileupload-ui-corner-top');
            this.element.find('.fileinput-button').each(function () {
                var fileInput = $(this).find('input:file').detach();
                $(this).button('destroy')
                    .append(fileInput);
        ...
        _create: function () {
            ...
            this._initTemplates();
            this.element
                .addClass('fileupload-ui-widget');
            this._initFileUploadButtonBar();
            /*this.element.find('.fileupload-content')
                .addClass('fileupload-ui-widget-content fileupload-ui-corner-bottom');*/
        ...
        destroy: function () {
            this.element.find('.fileupload-progressbar')
                .progressbar('destroy');
            /*this.element.find('.fileupload-content')
                .removeClass('fileupload-ui-widget-content fileupload-ui-corner-bottom');*/
            this._destroyFileUploadButtonBar();
            this.element.removeClass('fileupload-ui-widget');
